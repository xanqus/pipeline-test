            ...
            steps{
                sshagent(credentials : ["deploy-key"]) {
                    sh "ssh -o StrictHostKeyChecking=no ubuntu@${deployHost} \
                     'aws ecr get-login-password --region ${region} | docker login -u AWS -p $(aws ecr get-login-password --region ap-northeast-2) ${ecrUrl}; \
                      docker run -d -p 8088:8080 -t ${ecrUrl}/${repository}:${currentBuild.number};'"
                }
            }
        }
    }
